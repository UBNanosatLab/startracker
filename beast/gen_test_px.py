...
maxmag=np.min(result[:,1])
#255*10**((maxmag-i[1])/2.5) - generate some plausible pixel intensites
#using the magnitudes from our database. Use the brightest star as our 
#reference point for a pixel intensity of 255
if(MIN_MAG!=None):
	image_stars_info = [[i[5],i[6],255*10**((maxmag-i[1])/2.5)] for i in result if i[1]<=MIN_MAG]
else:
	image_stars_info = [[i[5],i[6],255*10**((maxmag-i[1])/2.5)] for i in result]
