...
int main (int argc, char** argv) {
	...
	size_t dbsize = mapsize*sizeof(int) + NUMCONST*sizeof(struct constelation);
	...
	//void *maptemp = malloc(dbsize);
	int poop[47249800];
	void *maptemp = &poop;
	map=(int*)maptemp;
	struct constelation *starptrtemp=(struct constelation*)(&map[mapsize]);
	starptr=starptrtemp;
	/*
	if (map == MAP_FAILED)
	{
	...
	*/
	memset(maptemp, -1, dbsize);
	cout<<map[0]<<endl<<flush;
	cout<<starptrtemp[0].last<<endl<<flush;
	cout<<map[0]<<endl<<flush;
	for (int curr_const=0;curr_const<NUMCONST;curr_const++){
		double temp1;
		for (int i=0;i<6;i++) {
			cin>>temp1;
			starptrtemp[curr_const].p[i]=temp1;
			cout<<starptrtemp[curr_const].p[i]<<flush;
		}
		for (int i=0;i<4;i++) {
			cin>>starptr[curr_const].s[i];
			cout<<starptr[curr_const].s[i]<<flush;
		}
