...
def resize((width, height)):
    ...
    glLoadIdentity()
    gluPerspective(6.0, float(width)/height, .1, 1000.)
...
#surface = pygame.display.set_mode(OPENGL|DOUBLEBUF)
resize((screenx,screeny))
init()

def main():
    EarthSurface = Image.open(os.path.join('Data', 'earth.jpg'))
    EarthData = EarthSurface.tobytes()

    glGenLists(1)
    glNewList(1, GL_COMPILE)#Self
    glScale(1, 1, -1)
    glBindTexture(GL_TEXTURE_2D, textures[0])
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, EarthSurface.size[0], EarthSurface.size[1], 0, GL_RGB, GL_UNSIGNED_BYTE, EarthData)
    ...
    for i in range(6) :
        texture_surface = Image.open(os.path.join('Data', files[i])).split()[0]
        width, height = texture_surface.size
        img = texture_surface.tobytes()
        glTexImage2D(faces[i], 0, GL_LUMINANCE, width, height, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, img)
