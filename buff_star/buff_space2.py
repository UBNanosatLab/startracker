...
def run():
    ...
    for line in starfile.readlines():
        ...
        points.append(Vector3(cos(radians(float(temp[2])))*cos(radians(float(temp[3]))),sin(radians(float(temp[2])))*cos(radians(float(temp[3]))),sin(radians(float(temp[3])))))
        #mag=pow(100,(minmag-float(temp[1]))/5-1)
        #print pos
    starfile.close()
    ...
    while True:
        ...
        for x, y, z in transformed_points:            
            ...
            if z < 0:
                ...
                pygame.draw.circle(screen, (255,255,255), (int(x-ball_center_x), int(y-ball_center_y)), 2, 0)
                #screen.blit(ball, (, ))
